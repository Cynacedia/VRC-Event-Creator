...
- Discord連携 - VRChatイベントと同時にDiscordイベントを自動作成（[設定ガイド](Discord%20Setup/DISCORD_SETUP.ja.md)）
...
Botトークン（Discord連携用）はOSのセキュアストレージを使用して暗号化されます。Discord API以外にトークンが送信されることはありません。
