...
## 特長
- グループごとにイベント詳細を自動入力するテンプレート。
- 繰り返しパターン生成（次回候補リスト + 手動の日時入力）。
- イベント自動化システム（実験的） - テンプレートパターンに基づいて自動的にイベントを投稿。
- グループカレンダー向けのイベント作成ウィザード。
- 今後のイベント用の「イベント編集」ビュー（グリッド + 編集モーダル）。
- 追加の権限を持つグループ向けの特別なイベントオプションに対応。
- プリセット付きテーマスタジオとUIカラーの完全制御（#RRGGBBAA対応）。
- 画像ID用のギャラリー選択・アップロード。
- システム起動時に開始 + システムトレイに最小化。
- 単一インスタンス保護で二重起動を防止。
- 初回起動時の言語選択付きローカライズ（en, fr, es, de, ja, zh, pt, ko, ru, nl）。
...
- Featured Event などの特別なトグルは特定のグループ権限が必要で、許可されている場合のみ表示されます。
