...
## 기능
- 그룹별 이벤트 상세를 자동으로 채우는 템플릿.
- 반복 패턴 생성기(다가오는 날짜 목록 + 수동 날짜/시간 입력).
- 이벤트 자동화 시스템(실험적) - 템플릿 패턴에 따라 자동으로 이벤트 게시.
- 그룹 캘린더 이벤트 생성 마법사.
- 예정 이벤트용 수정 보기(그리드 + 편집 모달).
- 추가 권한이 있는 그룹을 위한 특별한 이벤트 옵션을 지원.
...
- 첫 실행 언어 선택이 있는 현지화(en, fr, es, de, ja, zh, pt, ko, ru, nl).
...
봇 토큰(Discord 연동용)은 OS의 보안 저장소를 사용하여 암호화됩니다. Discord API 외에는 어디로도 전송되지 않습니다.

__**캐시 파일이나 앱 데이터 폴더를 공유하지 마세요.**__

## 사용 참고사항
- 템플릿에는 템플릿 이름, 이벤트 이름, 설명이 필요합니다.
